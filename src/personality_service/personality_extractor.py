...
from ..models.personality_models import RPGPersonality, VernacularPattern, PersonalityTrait, PersonalityPrompt
...
class PersonalityExtractor:
    ...
    def _extract_vernacular_patterns(self, chunks: List[ContentChunk]) -> List[VernacularPattern]:
        ...
        for term, info in unique_terms.items():
    ...
    def _extract_system_context(self, chunks: List[ContentChunk]) -> str:
        ...
        setting_keywords = re.findall(
            r'\\b(world|setting|universe|realm|dimension|plane|city|kingdom|empire|age|era|time|period)\\b',
            " ".join([chunk.content for chunk in chunks[:10]])  # First 10 chunks
        )
        
        # Generic description based on common terms
        all_text = " ".join([chunk.content for chunk in chunks])
